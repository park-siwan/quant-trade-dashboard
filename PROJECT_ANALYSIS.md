...
## 7. MTF (Multi-Timeframe) 분석

### 7.1 MTF Overview 대시보드

모든 타임프레임(5m, 15m, 30m, 1h, 4h, 1d) 상태를 한눈에 확인:

| 열 | 설명 |
|---|---|
| **TF** | 타임프레임 |
| **Trend** | 추세 (🟢 상승, 🔴 하락, 🟡 중립) |
| **RSI** | RSI 값 (30↓=과매도, 70↑=과매수) |
| **CVD** | CVD 방향 (최근 10캔들 기준 ±2%) |
| **OI** | OI 방향 (최근 10캔들 기준 ±2%) |
| **Div** | 최근 다이버전스 신호 |
...
### 7.2 정렬 점수 (Alignment Score)

모든 타임프레임이 같은 방향일수록 높은 점수:
- **80%+**: 강한 정렬 → 추세 추종 유리
- **50~80%**: 중간 → 신중한 진입
- **50% 미만**: 혼조 → 관망 권장
...
규칙:
- 4시간봉 추세가 반대 → Invalid (신뢰도 0%)
- 모든 상위 TF 정렬 → Strong Entry (신뢰도 100%)
- 일부 정렬 → 정렬 비율에 따른 신뢰도
```
